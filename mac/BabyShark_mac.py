...
def ouvrir():
    ...
    for i in range(nb_trames):
        ...
        if(valid_frame==[]):#si la trame est erronée
            ...
            button.pack(fill=X)
    f.close()
##creer des boutons pour charger les fichiers
fichier= Button(window, highlightbackground='grey', text="Charger le fichier", command=ouvrir)
quitter= Button(window, highlightbackground='grey', text="Quitter", command=window.quit)
quitter.pack(side=BOTTOM, pady=20)
fichier.pack(side=BOTTOM ,pady=10)
window.mainloop()
